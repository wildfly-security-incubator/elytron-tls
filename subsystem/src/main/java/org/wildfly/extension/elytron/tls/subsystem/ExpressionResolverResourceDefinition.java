...
import static org.wildfly.extension.elytron.tls.subsystem.Capabilities.CREDENTIAL_STORE_CAPABILITY;
import static org.wildfly.extension.elytron.tls.subsystem.Capabilities.EXPRESSION_RESOLVER_CAPABILITY;
...
import org.wildfly.extension.elytron.tls.subsystem.expression.ElytronExpressionResolver;
import org.wildfly.extension.elytron.tls.subsystem.expression.ElytronExpressionResolver.ResolverConfiguration;
import org.wildfly.extension.elytron.tls.subsystem.expression.ExpressionResolverRuntimeHandler;
...
class ExpressionResolverResourceDefinition extends SimpleResourceDefinition {
    ...
    private static final StandardResourceDescriptionResolver RESOURCE_RESOLVER =
            ElytronTlsExtension.getResourceDescriptionResolver(Constants.EXPRESSION, Constants.ENCRYPTION);
    ...
    private static final SimpleAttributeDefinition NAME = new SimpleAttributeDefinitionBuilder(Constants.NAME, ModelType.STRING, false)
            .setAllowExpression(false)
            .setMinSize(1)
            .setRestartAllServices()
            .build();

    private static final SimpleAttributeDefinition CREDENTIAL_STORE = new SimpleAttributeDefinitionBuilder(Constants.CREDENTIAL_STORE, ModelType.STRING, false)
            .setAllowExpression(false)
            .setMinSize(1)
            .setRestartAllServices()
    ...
    private static final SimpleAttributeDefinition SECRET_KEY = new SimpleAttributeDefinitionBuilder(Constants.SECRET_KEY, ModelType.STRING, false)
            .setAllowExpression(false)
            .setMinSize(1)
            .setRestartAllServices()
            .build();

    private static final ObjectTypeAttributeDefinition RESOLVER = new ObjectTypeAttributeDefinition.Builder(Constants.RESOLVER, NAME, CREDENTIAL_STORE, SECRET_KEY)
            .setRequired(true)
            .setRestartAllServices()
            .build();

    static final ObjectListAttributeDefinition RESOLVERS = new ObjectListAttributeDefinition.Builder(Constants.RESOLVERS, RESOLVER)
            .setRequired(true)
            .setMinSize(1)
    ...
    static final SimpleAttributeDefinition DEFAULT_RESOLVER = new SimpleAttributeDefinitionBuilder(Constants.DEFAULT_RESOLVER, ModelType.STRING, true)
            .setAllowExpression(false)
            .setMinSize(1)
            .setRestartAllServices()
            .build();

    static final SimpleAttributeDefinition PREFIX = new SimpleAttributeDefinitionBuilder(Constants.PREFIX, ModelType.STRING, true)
            .setAllowExpression(false)
    ...
    static final SimpleAttributeDefinition RESOLVER_PARAM = new SimpleAttributeDefinitionBuilder(Constants.RESOLVER, ModelType.STRING, true)
            .setMinSize(1)
            .build();

    static final SimpleAttributeDefinition CLEAR_TEXT = new SimpleAttributeDefinitionBuilder(Constants.CLEAR_TEXT, ModelType.STRING, false)
            .setMinSize(1)
            .build();

    static final SimpleOperationDefinition CREATE_EXPRESSION = new SimpleOperationDefinitionBuilder(Constants.CREATE_EXPRESSION, RESOURCE_RESOLVER)
    ...
    ExpressionResolverResourceDefinition(OperationStepHandler add, OperationStepHandler remove,
        ...
        super(new Parameters(PathElement.pathElement(Constants.EXPRESSION, Constants.ENCRYPTION),
                RESOURCE_RESOLVER)
                .setAddHandler(add)
    ...
    static ResourceDefinition getExpressionResolverDefinition(PathAddress parentAddress, AtomicReference<ExpressionResolverExtension> resolverRef) {
        final PathAddress resourceAddress = parentAddress.append(PathElement.pathElement(Constants.EXPRESSION, Constants.ENCRYPTION));
    ...
    private static class CreateExpressionHandler extends ElytronRuntimeOnlyHandler {
        ...
        protected void executeRuntimeStep(OperationContext context, ModelNode operation) throws OperationFailedException {
            ...
            context.getResult().get(Constants.EXPRESSION).set(expression);
        }
