...
import static org.wildfly.extension.elytron.tls.ProviderAttributeDefinition.LOADED_PROVIDER;
import static org.wildfly.extension.elytron.tls.ProviderAttributeDefinition.populateProvider;
...
import org.wildfly.extension.elytron.tls.KeyStoreService.LoadKey;
...
final class KeyStoreDefinition extends SimpleResourceDefinition {
    ...
    private static final SimpleAttributeDefinition SIZE = new SimpleAttributeDefinitionBuilder(Constants.SIZE, ModelType.INT)
        .setStorageRuntime()
        .build();

    private static final SimpleAttributeDefinition SYNCHRONIZED = new SimpleAttributeDefinitionBuilder(Constants.SYNCHRONIZED, ModelType.STRING)
        .setStorageRuntime()
        .build();

    private static final SimpleAttributeDefinition MODIFIED = new SimpleAttributeDefinitionBuilder(Constants.MODIFIED, ModelType.BOOLEAN)
        .setStorageRuntime()
        .build();

    // Operations

    private static final SimpleOperationDefinition LOAD = new SimpleOperationDefinitionBuilder(Constants.LOAD, RESOURCE_RESOLVER)
        .setRuntimeOnly()
        .build();

    private static final SimpleOperationDefinition STORE = new SimpleOperationDefinitionBuilder(Constants.STORE, RESOURCE_RESOLVER)
        .setRuntimeOnly()
        .build();
    ...
    abstract static class KeyStoreRuntimeOnlyHandler extends ElytronRuntimeOnlyHandler {
        ...
        protected void executeRuntimeStep(OperationContext context, ModelNode operation) throws OperationFailedException {
            ...
            if ((serviceState = serviceContainer.getState()) != State.UP) {
                if (serviceMustBeUp) {
                    throw LOGGER.requiredServiceNotUp(keyStoreName, serviceState);
                }
                return;
            }
