...
class ModifiableKeyStoreDecorator extends DelegatingResourceDefinition {
    ...
    static class ReadAliasesHandler extends ElytronRuntimeOnlyHandler {
        ...
        static final SimpleAttributeDefinition RECURSIVE = new SimpleAttributeDefinitionBuilder(Constants.RECURSIVE, ModelType.BOOLEAN, true)
                .setAllowExpression(false)
                .setDefaultValue(ModelNode.FALSE)
                .build();

        static final SimpleAttributeDefinition VERBOSE = new SimpleAttributeDefinitionBuilder(Constants.VERBOSE, ModelType.BOOLEAN, true)
                .setAllowExpression(false)
        ...
        static void register(ManagementResourceRegistration resourceRegistration, ResourceDescriptionResolver descriptionResolver) {
            SimpleOperationDefinition READ_ALIASES = new SimpleOperationDefinitionBuilder(Constants.READ_ALIASES, descriptionResolver)
    ...
    static class ReadAliasHandler extends ElytronRuntimeOnlyHandler {
        ...
        static void register(ManagementResourceRegistration resourceRegistration, ResourceDescriptionResolver descriptionResolver) {
            SimpleOperationDefinition READ_ALIAS = new SimpleOperationDefinitionBuilder(Constants.READ_ALIAS, descriptionResolver)
        ...
        private static void readAlias(final KeyStore keyStore, final String alias, final boolean verbose,
            ...
            if (!keyStore.containsAlias(alias)) {
                LOGGER.tracef("Alias [%s] does not exists in KeyStore");
                return;
            }

            result.get(Constants.ALIAS).set(alias);
            result.get(Constants.ENTRY_TYPE).set(getEntryType(keyStore, alias));

            Date creationDate = keyStore.getCreationDate(alias);
            if (creationDate != null) {
                SimpleDateFormat sdf = new SimpleDateFormat(ISO_8601_FORMAT);
                result.get(Constants.CREATION_DATE).set(sdf.format(creationDate));
            }
            ...
            if (chain == null) {
                ...
                if (cert != null) {
                    writeCertificate(result.get(Constants.CERTIFICATE), cert, verbose);
                }
            } else {
                writeCertificates(result.get(Constants.CERTIFICATE_CHAIN), chain, verbose);
            }
        }
    }
    ...
    static class RemoveAliasHandler extends ElytronRuntimeOnlyHandler {
        ...
        static void register(ManagementResourceRegistration resourceRegistration, ResourceDescriptionResolver descriptionResolver) {
            resourceRegistration.registerOperationHandler(new SimpleOperationDefinitionBuilder(Constants.REMOVE_ALIAS, descriptionResolver)
                        .setParameters(ALIAS)
                        .setRuntimeOnly()
                        .build()
