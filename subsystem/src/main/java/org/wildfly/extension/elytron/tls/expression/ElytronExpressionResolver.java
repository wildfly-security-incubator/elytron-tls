...
package org.wildfly.extension.elytron.tls.expression;
...
public class ElytronExpressionResolver implements ExpressionResolverExtension {
    ...
    private String resolveExpressionInternal(String fullExpression, OperationContext operationContext, CapabilityServiceSupport serviceSupport) {
        ...
        if (fullExpression.length() > 3) {
            ...
            if (expression.startsWith(completePrefix)) {
                ...
                if (resolver == null) {
                    throw LOGGER.expressionResolutionWithoutResolver(fullExpression);
                }

                ResolverConfiguration resolverConfiguration = resolverConfigurations.get(resolver);
                if (resolverConfiguration == null) {
                    throw LOGGER.invalidResolver(fullExpression);
                }

                LOGGER.tracef("Attempting to decrypt expression '%s' using credential store '%s' and alias '%s'.",
                ...
                try {
                    ...
                    throw LOGGER.unableToLoadCredential(e);
                }
                ...
                try {
                    ...
                    throw LOGGER.unableToDecryptExpression(fullExpression, e);
                }
            }
        }
        return null;
    ...
    public String createExpression(final String resolver, final String clearText, OperationContext context) throws OperationFailedException {
        ...
        if (resolvedResolver == null) {
            throw LOGGER.noResolverSpecifiedAndNoDefault();
        }

        ResolverConfiguration resolverConfiguration = resolverConfigurations.get(resolvedResolver);
        if (resolverConfiguration == null) {
            throw LOGGER.noResolverWithSpecifiedName(resolvedResolver);
        }
        ...
        try {
            ...
            if (credential == null) {
                throw LOGGER.credentialDoesNotExist(resolverConfiguration.getAlias(), SecretKeyCredential.class.getSimpleName());
            }
            secretKey = credential.getSecretKey();
        } catch (CredentialStoreException e) {
            throw new OperationFailedException(LOGGER.unableToLoadCredential(e));
        }
        ...
        try {
            ...
            throw LOGGER.unableToEncryptClearText(e);
        }

        return resolver == null ? String.format("${%s::%s}", prefix, cipherTextToken)
    ...
    void ensureInitialised(String initialisingFor, OperationContext context) throws OperationFailedException {
        if (!initialised) {
            ...
            if (firstFailure != null) {
                ...
                throw LOGGER.expressionResolverInitialisationAlreadyFailed(firstFailure);
            }

            if (initialisingFor != null) {
                String existingInitialisation = this.initialisingFor.get();
                if (existingInitialisation != null) {
                    throw LOGGER.cycleDetectedInitialisingExpressionResolver(existingInitialisation,
                ...
                if (!initialised) {
                    try {
                        ...
                        if (context == null) {
                            ...
                            throw LOGGER.illegalNonManagementInitialization(getClass());
                        }
    ...
    private CredentialStore resolveCredentialStore(String credentialStore, OperationContext operationContext, CapabilityServiceSupport serviceSupport) {
        ...
        try {
            if (operationContext != null) {
                try {
                    ...
                    if (operationContext.getCurrentStage() == OperationContext.Stage.MODEL) {
                        ...
                        throw LOGGER.modelStageResolutionNotSupported(re);
                    } else {
                        throw re;
                    }
                }
            ...
            if (e instanceof OperationClientException) {
                ...
                toThrow = LOGGER.unableToInitializeCredentialStore(credentialStore, e.getLocalizedMessage(), e);
            } else {
                // Use ExpressionResolutionServerException to wrap other failures
                toThrow = LOGGER.unableToResolveCredentialStore(credentialStore, e.getLocalizedMessage(), e);
            }
        }
