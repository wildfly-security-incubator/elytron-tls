...
public class CertificateAuthoritiesTestCase extends AbstractSubsystemTest {
    ...
    public void testCreateAccount() throws Exception {
        ...
        try {
            ...
            operation.get(ClientConstants.OP).set(Constants.CREATE_ACCOUNT);
    ...
    public void testCreateAccountWithCustomCAWithStaging() throws Exception {
        ...
        try {
            ...
            operation.get(ClientConstants.OP).set(Constants.CREATE_ACCOUNT);
            operation.get(Constants.AGREE_TO_TERMS_OF_SERVICE).set(true);
            operation.get(Constants.STAGING).set(true);
            assertSuccess(services.executeOperation(operation));
            assertEquals(NEW_ACCT_LOCATION, acmeAccount.getAccountUrl());
        } finally {
            removeCertificateAuthorityAccount();
    ...
    public void testCreateAccountWithEmptyStagingUrlAndStagingValueTrue() throws Exception {
        ...
        try {
            ...
            assertTrue(failureDescription.contains("ELYTLS00317") && failureDescription.contains("ELY10057"));
        } finally {
            removeCertificateAuthorityAccount();
            removeCertificateAuthority();
            removeKeyStore(ACCOUNTS_KEYSTORE_NAME);
    ...
    public void testCreateAccountWithoutAgreeingToTermsOfService() throws Exception {
        ...
        try {
            ...
            operation.get(ClientConstants.OP_ADDR).add("subsystem", "elytron-tls").add("certificate-authority-account", CERTIFICATE_AUTHORITY_ACCOUNT_NAME);
            operation.get(ClientConstants.OP).set(Constants.CREATE_ACCOUNT);
            operation.get(Constants.AGREE_TO_TERMS_OF_SERVICE).set(false);
            ModelNode result = services.executeOperation(operation);
            assertFailed(result);
            String failureDescription = result.get(FAILURE_DESCRIPTION).asString();
            assertTrue(failureDescription.contains("ELYTLS00317") && failureDescription.contains("must agree to terms of service"));
        } finally {
            removeCertificateAuthorityAccount();
            removeCertificateAuthority();
            removeKeyStore(ACCOUNTS_KEYSTORE_NAME);
    ...
    public void testUpdateAccount() throws Exception {
        ...
        try {
            ModelNode operation = new ModelNode();
            operation.get(ClientConstants.OP_ADDR).add("subsystem", "elytron-tls").add("certificate-authority-account", CERTIFICATE_AUTHORITY_ACCOUNT_NAME);
            operation.get(ClientConstants.OP).set(Constants.UPDATE_ACCOUNT);
            operation.get(Constants.AGREE_TO_TERMS_OF_SERVICE).set(false);
            assertSuccess(services.executeOperation(operation));
        } finally {
            removeCertificateAuthorityAccount();
    ...
    public void testDeactivateAccount() throws Exception {
        ...
        try {
            ModelNode operation = new ModelNode();
            operation.get(ClientConstants.OP_ADDR).add("subsystem", "elytron-tls").add("certificate-authority-account", CERTIFICATE_AUTHORITY_ACCOUNT_NAME);
            operation.get(ClientConstants.OP).set(Constants.DEACTIVATE_ACCOUNT);
            assertSuccess(services.executeOperation(operation));
        } finally {
            removeCertificateAuthorityAccount();
            removeCertificateAuthority();
    ...
    public void testGetMetadataAllValuesSet() throws Exception {
        ...
        try {
            ModelNode operation = new ModelNode();
            operation.get(ClientConstants.OP_ADDR).add("subsystem", "elytron-tls").add("certificate-authority-account", CERTIFICATE_AUTHORITY_ACCOUNT_NAME);
            operation.get(ClientConstants.OP).set(Constants.GET_METADATA);
            ModelNode result = assertSuccess(services.executeOperation(operation)).get(ClientConstants.RESULT);
            assertEquals("https://boulder:4431/terms/v7", result.get(Constants.TERMS_OF_SERVICE).asString());
            assertEquals("https://github.com/letsencrypt/boulder", result.get(Constants.WEBSITE).asString());
            List<ModelNode> caaIdentities = result.get(Constants.CAA_IDENTITIES).asList();
            ...
            assertTrue(result.get(Constants.EXTERNAL_ACCOUNT_REQUIRED).asBoolean());
        } finally {
            removeCertificateAuthorityAccount();
            removeCertificateAuthority();
            removeKeyStore(ACCOUNTS_KEYSTORE_NAME);
    ...
    public void testGetMetadataSomeValuesSet() throws Exception {
        ...
        try {
            ...
            ModelNode result = assertSuccess(services.executeOperation(operation)).get(ClientConstants.RESULT);
            assertEquals("https://boulder:4431/terms/v7", result.get(Constants.TERMS_OF_SERVICE).asString());
            assertEquals(ModelType.UNDEFINED, result.get(Constants.WEBSITE).getType());
            assertEquals(ModelType.UNDEFINED, result.get(Constants.CAA_IDENTITIES).getType());
            assertFalse(result.get(Constants.EXTERNAL_ACCOUNT_REQUIRED).asBoolean());
        } finally {
            removeCertificateAuthorityAccount();
            removeCertificateAuthority();
            removeKeyStore(ACCOUNTS_KEYSTORE_NAME);
    ...
    public void testGetMetadataNoValuesSet() throws Exception {
        ...
        try {
            ...
            operation.get(ClientConstants.OP).set(Constants.GET_METADATA);
            ModelNode result = assertSuccess(services.executeOperation(operation)).get(ClientConstants.RESULT);
            assertEquals(ModelType.UNDEFINED, result.get(Constants.TERMS_OF_SERVICE).getType());
            assertEquals(ModelType.UNDEFINED, result.get(Constants.WEBSITE).getType());
            assertEquals(ModelType.UNDEFINED, result.get(Constants.CAA_IDENTITIES).getType());
            assertEquals(ModelType.UNDEFINED, result.get(Constants.EXTERNAL_ACCOUNT_REQUIRED).getType());
        } finally {
            removeCertificateAuthorityAccount();
            removeCertificateAuthority();
            removeKeyStore(ACCOUNTS_KEYSTORE_NAME);
    ...
    public void testAddCertificateAuthorityWithLetsEncryptName() {
        ModelNode operation = new ModelNode();
        operation.get(ClientConstants.OP_ADDR).add("subsystem", "elytron-tls").add("certificate-authority", CertificateAuthority.LETS_ENCRYPT.getName());
        operation.get(ClientConstants.OP).set(ClientConstants.ADD);
        operation.get(Constants.URL).set(CERTIFICATE_AUTHORITY_TEST_URL);
        assertFailed(services.executeOperation(operation));
    }
    ...
    public void testAddCertificateAuthorityWithStagingUrl() {
        ...
        try {
            ...
            ModelNode result = assertSuccess(services.executeOperation(operation)).get(ClientConstants.RESULT);
            assertEquals(SIMULATED_LETS_ENCRYPT_ENDPOINT, result.get(Constants.URL).asString());
            assertEquals(SIMULATED_LETS_ENCRYPT_ENDPOINT, result.get(Constants.STAGING_URL).asString());
        } finally {
            removeCertificateAuthority();
        }
    }
    ...
    public void testAddCertificateAuthorityWithEmptyStagingUrl() {
        ...
        try {
            ...
            ModelNode result = assertSuccess(services.executeOperation(operation)).get(ClientConstants.RESULT);
            assertEquals(SIMULATED_LETS_ENCRYPT_ENDPOINT, result.get(Constants.URL).asString());
            assertEquals(ModelType.UNDEFINED, result.get(Constants.STAGING_URL).getType());
        } finally {
            removeCertificateAuthority();
        }
    }
    ...
    private void changeCertificateAuthorityInCertificateAuthorityAccount() {
        ...
        operation.get(ClientConstants.NAME).set(Constants.CERTIFICATE_AUTHORITY);
    ...
    private void addCertificateAuthorityWithStagingUrl() {
        ...
        operation.get(ClientConstants.OP).set(ClientConstants.ADD);
        operation.get(Constants.URL).set(SIMULATED_LETS_ENCRYPT_ENDPOINT);
        operation.get(Constants.STAGING_URL).set(SIMULATED_LETS_ENCRYPT_ENDPOINT);
        assertSuccess(services.executeOperation(operation));
    }
    ...
    private void checkCertificateAuthorityIs(String certificateAuthorityName) {
        ...
        operation.get(ClientConstants.OP).set(ClientConstants.READ_RESOURCE_OPERATION);
        ModelNode result = assertSuccess(services.executeOperation(operation)).get(ClientConstants.RESULT);
        assertEquals(certificateAuthorityName, result.get(Constants.CERTIFICATE_AUTHORITY).asString());
    }
    ...
    private void addCertificateAuthorityAccountWithCustomCA(String alias, String[] contactUrlsList) throws Exception {
        ...
        operation.get(Constants.CONTACT_URLS).set(contactUrls);
        operation.get(Constants.CERTIFICATE_AUTHORITY).set(CERTIFICATE_AUTHORITY_NAME);
        operation.get(Constants.KEY_STORE).set(ACCOUNTS_KEYSTORE_NAME);
        operation.get(Constants.ALIAS).set(alias);
        operation.get(CredentialReference.CREDENTIAL_REFERENCE).get(CredentialReference.CLEAR_TEXT).set(KEYSTORE_PASSWORD);
