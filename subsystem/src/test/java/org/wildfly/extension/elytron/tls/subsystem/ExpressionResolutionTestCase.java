...
public class ExpressionResolutionTestCase extends AbstractSubsystemBaseTest {
    ...
    public void testExpressionEncryptionOperations() throws Exception {
        ...
        add.get(ClientConstants.OP_ADDR).add("subsystem", "elytron-tls").add(Constants.SECRET_KEY_CREDENTIAL_STORE, "my-test-store");
        ...
        generate.get(ClientConstants.OP_ADDR).add("subsystem", "elytron-tls").add(Constants.SECRET_KEY_CREDENTIAL_STORE, "my-test-store");
        generate.get(ClientConstants.OP).set(Constants.GENERATE_SECRET_KEY);
        generate.get(Constants.ALIAS).set("TestKeyOne");

        assertSuccess(services.executeOperation(generate));

        ModelNode export = new ModelNode();
        export.get(ClientConstants.OP_ADDR).add("subsystem", "elytron-tls").add(Constants.SECRET_KEY_CREDENTIAL_STORE, "my-test-store");
        export.get(ClientConstants.OP).set(Constants.EXPORT_SECRET_KEY);
        export.get(Constants.ALIAS).set("TestKeyOne");
        ...
        importOP.get(ClientConstants.OP_ADDR).add("subsystem", "elytron-tls").add(Constants.SECRET_KEY_CREDENTIAL_STORE, "my-test-store");
        importOP.get(ClientConstants.OP).set(Constants.IMPORT_SECRET_KEY);
        importOP.get(Constants.ALIAS).set("TestKeyTwo");
        importOP.get(Constants.KEY).set(SecretKeyUtil.exportSecretKey(testKeyTwo));

        assertSuccess(services.executeOperation(importOP));

          // Test read-aliases
        testExpectedAliases(services, Constants.SECRET_KEY_CREDENTIAL_STORE, "my-test-store", "testkeyone", "testkeytwo");

        // Define an expression=encryption resource with three resolvers (one for each key and one for a missing key)
        add = new ModelNode();
        add.get(ClientConstants.OP_ADDR).add("subsystem", "elytron-tls").add(Constants.EXPRESSION, Constants.ENCRYPTION);
        add.get(ClientConstants.OP).set(ClientConstants.ADD);
        ...
        add.get(Constants.RESOLVERS).set(resolvers);

        add.get(Constants.DEFAULT_RESOLVER).set("ResolverTwo");
    ...
    private static void testCreateExpression(KernelServices services, SecretKey secretKey, String resolver) throws Exception {
        ...
        createExpression.get(ClientConstants.OP_ADDR).add("subsystem", "elytron-tls").add(Constants.EXPRESSION, Constants.ENCRYPTION);
        createExpression.get(ClientConstants.OP).set(Constants.CREATE_EXPRESSION);
        if (resolver != null) {
            createExpression.get(Constants.RESOLVER).set(resolver);
        }
        ...
        if (secretKey != null) {
            ...
            String expression = result.get(ClientConstants.RESULT).get(Constants.EXPRESSION).asString();
            ...
            assertTrue("Expected Error Code", result.get(ClientConstants.FAILURE_DESCRIPTION).asString().contains("ELYTLS00920:"));
        }
    }
    ...
    private static ModelNode resolver(final String name, final String credentialStore, final String secretKey) {
        ...
        resolver.get(Constants.NAME).set(name);
        resolver.get(Constants.CREDENTIAL_STORE).set(credentialStore);
        resolver.get(Constants.SECRET_KEY).set(secretKey);
        return resolver;
    }
    ...
    private static String emptySubsystemXml() {
        ...
        sb.append(ElytronTlsExtension.CURRENT_NAMESPACE);
        sb.append("\"></subsystem>");
    ...
    public void testExpressionEncryptionCycle() throws Exception {
        ...
        addEE.get(ClientConstants.OP_ADDR).add("subsystem", "elytron-tls").add(Constants.EXPRESSION, Constants.ENCRYPTION);
        ...
        assertTrue("Expected Error Code (Cycle Detected)",
                result.get(ClientConstants.FAILURE_DESCRIPTION).asString().contains("ELYTLS00043:"));
    ...
    private CredentialStoreUtility createCredentialStoreUtility(final String path, final ModelNode steps, final String storeName,
        ...
        ModelNode add = new ModelNode();
        add.get(ClientConstants.OP_ADDR).add("subsystem", "elytron-tls").add(Constants.CREDENTIAL_STORE, storeName);
        add.get(ClientConstants.OP).set(ClientConstants.ADD);
        add.get(Constants.PATH).set(path);
